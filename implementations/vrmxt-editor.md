...
| Warudo | Done | — | Done (Manager templates; textures omitted) |
...
| Warudo | Done | — | Done (ModHost `.mat` templates; strip texture props; no Materialize) |
...
| Transfer | — (PropertyGroups authored directly) | Done (Sync from Override Material **asset**; variant survival) | Planned (desktop; from `.mat` only) | Done (Manager: StreamingAssets `.mat` YAML values only; no shader/textures) |
...
**Apply Done**, **Transfer Done** (Manager ModHost templates; texture props omitted),
...
| Warudo material templates | Done (StreamingAssets `.mat` YAML → values only; shader GUID/textures skipped) |
