...
| UI | Readonly Material PROPERTIES panel. Shows parsed engine / identity / bindings / properties. If JSON is stored but unparsed, shows `Stored (unparsed)`. No authoring. |
...
`format/materials_override.py` (`idType` / `id`, Unreal `idType: "materialSet"`,
optional `properties[]`). Panel: `materials_override/panel.py`.
...
- [ ] Authoring UI (edit / create overrides in Blender)
