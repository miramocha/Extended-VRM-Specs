...
- **Fatal tells here**: marketing puffery ("seamless interoperability", "unlock the full potential"), "simply" before non-trivial steps, negative parallelism in motivation sections, invented rationale or version history, vague "should work with most tools".
- **De-slop moves**: state MUST/SHOULD/MAY (or equivalent) once and mean it. Name schemas, fields, units, and compatibility targets. Prefer "Extension X adds field Y of type Z" over essay framing. Keep tables and field lists; strip narrative wrapping. Do not invent citations to VRM/glTF specs — link real sections or leave `[ADD: …]`.
- **Cadence**: short normative sentences are good. Uniform 20-word paragraphs of motivation prose are not — cut throat-clearing, keep the requirement.
